...
{
	#if DEBUG 
	public class tst_fetch_dadosLoja : tst_base
	{
/// USER [ usr_var_decl ]
/// USER [ usr_var_decl ] END

		public fetch_dadosLoja transaction = new fetch_dadosLoja();
		...
		public tst_fetch_dadosLoja ( ref StreamWriter p_m_Log, 
		...
		public bool Test()
		{
			if ( demand == "!" || demand == "fetch_dadosLoja" )
			{
			...
			{
				result = " fetch_dadosLoja                    " + " ## no available coverage! ";
				currentItem++;
				return false;
			}
		...
		public void call_fetch_dadosLoja ( string st_cnpj, string st_cod, ref CNetHeader header ) 
		{
			m_Comm.Clear();
...
/// USER [ custom_functions ] END
		...
		public bool Item1()
		{
			...
			tst_unit.LogTest ( "fetch_dadosLoja Item1", ref m_Log );
...
/// USER [ setup_test_1 ] END
			...
			call_fetch_dadosLoja ( st_cnpj, st_cod, ref  header ); 
