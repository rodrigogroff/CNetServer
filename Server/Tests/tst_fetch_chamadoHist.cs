...
{
	#if DEBUG 
	public class tst_fetch_chamadoHist : tst_base
	{
/// USER [ usr_var_decl ]
/// USER [ usr_var_decl ] END

		public fetch_chamadoHist transaction = new fetch_chamadoHist();
		...
		public tst_fetch_chamadoHist ( ref StreamWriter p_m_Log, 
		...
		public bool Test()
		{
			if ( demand == "!" || demand == "fetch_chamadoHist" )
			{
			...
			{
				result = " fetch_chamadoHist                  " + " ## no available coverage! ";
				currentItem++;
				return false;
			}
		...
		public void call_fetch_chamadoHist ( string id_chamado, ref CNetHeader header ) 
		{
			m_Comm.Clear();
...
/// USER [ custom_functions ] END
		...
		public bool Item1()
		{
			...
			tst_unit.LogTest ( "fetch_chamadoHist Item1", ref m_Log );
...
/// USER [ setup_test_1 ] END
			...
			call_fetch_chamadoHist ( id_chamado, ref  header ); 
...
/// USER [ execute_1 ] END
			...
			recv_fetch_chamadoHist ( ref dc, ref lst ); 
