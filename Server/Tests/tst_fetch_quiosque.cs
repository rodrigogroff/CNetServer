...
{
	#if DEBUG 
	public class tst_fetch_quiosque : tst_base
	{
/// USER [ usr_var_decl ]
/// USER [ usr_var_decl ] END

		public fetch_quiosque transaction = new fetch_quiosque();
		...
		public tst_fetch_quiosque ( ref StreamWriter p_m_Log, 
		...
		public bool Test()
		{
			if ( demand == "!" || demand == "fetch_quiosque" )
			{
			...
			{
				result = " fetch_quiosque                     " + " ## no available coverage! ";
				currentItem++;
				return false;
			}
...
/// USER [ custom_functions ] END
		...
		public bool Item1()
		{
			...
			tst_unit.LogTest ( "fetch_quiosque Item1", ref m_Log );
...
/// USER [ setup_test_1 ] END
			...
			call_fetch_quiosque ( st_empresa, ref  header ); 
