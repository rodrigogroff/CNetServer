...
{
	#if DEBUG 
	public class tst_fetch_user_lojista : tst_base
	{
/// USER [ usr_var_decl ]
/// USER [ usr_var_decl ] END

		public fetch_user_lojista transaction = new fetch_user_lojista();
		...
		public tst_fetch_user_lojista ( ref StreamWriter p_m_Log, 
		...
		public bool Test()
		{
			if ( demand == "!" || demand == "fetch_user_lojista" )
			{
			...
			{
				result = " fetch_user_lojista                 " + " ## no available coverage! ";
				currentItem++;
				return false;
			}
...
/// USER [ custom_functions ] END
		...
		public bool Item1()
		{
			...
			tst_unit.LogTest ( "fetch_user_lojista Item1", ref m_Log );
...
/// USER [ setup_test_1 ] END
			...
			call_fetch_user_lojista ( ref  header ); 
...
/// USER [ execute_1 ] END
			...
			recv_fetch_user_lojista ( ref lst ); 
