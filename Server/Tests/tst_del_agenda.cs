...
{
	#if DEBUG 
	public class tst_del_agenda : tst_base
	{
/// USER [ usr_var_decl ]
/// USER [ usr_var_decl ] END

		public del_agenda transaction = new del_agenda();
		...
		public tst_del_agenda ( ref StreamWriter p_m_Log, 
		...
		public bool Test()
		{
			...
			{
				result = " del_agenda                         " + " ## no available coverage! ";
				currentItem++;
				return false;
			}
		...
		public void call_del_agenda ( string fk_agenda, ref CNetHeader header ) 
		{
			m_Comm.Clear();
...
/// USER [ custom_functions ] END
		...
		public bool Item1()
		{
			...
			tst_unit.LogTest ( "del_agenda Item1", ref m_Log );
...
/// USER [ setup_test_1 ] END
			...
			call_del_agenda ( fk_agenda, ref  header ); 
