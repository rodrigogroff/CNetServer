...
{
	#if DEBUG 
	public class tst_fetch_termLoja : tst_base
	{
/// USER [ usr_var_decl ]
/// USER [ usr_var_decl ] END

		public fetch_termLoja transaction = new fetch_termLoja();
		...
		public tst_fetch_termLoja ( ref StreamWriter p_m_Log, 
		...
		public bool Test()
		{
			if ( demand == "!" || demand == "fetch_termLoja" )
			{
			...
			{
				result = " fetch_termLoja                     " + " ## no available coverage! ";
				currentItem++;
				return false;
			}
		...
		public void recv_fetch_termLoja ( ref ArrayList lst )
		{
...
/// USER [ custom_functions ] END
		...
		public bool Item1()
		{
			...
			tst_unit.LogTest ( "fetch_termLoja Item1", ref m_Log );
...
/// USER [ setup_test_1 ] END
			...
			call_fetch_termLoja ( st_cod_loja, ref  header ); 
