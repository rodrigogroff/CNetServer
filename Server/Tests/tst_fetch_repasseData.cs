...
{
	#if DEBUG 
	public class tst_fetch_repasseData : tst_base
	{
/// USER [ usr_var_decl ]
/// USER [ usr_var_decl ] END

		public fetch_repasseData transaction = new fetch_repasseData();
		...
		public tst_fetch_repasseData ( ref StreamWriter p_m_Log, 
		...
		public bool Test()
		{
			if ( demand == "!" || demand == "fetch_repasseData" )
			{
			...
			{
				result = " fetch_repasseData                  " + " ## no available coverage! ";
				currentItem++;
				return false;
			}
...
/// USER [ custom_functions ] END
		...
		public bool Item1()
		{
			...
			tst_unit.LogTest ( "fetch_repasseData Item1", ref m_Log );
...
/// USER [ setup_test_1 ] END
			...
			call_fetch_repasseData ( dt_ini, ref  header ); 
...
/// USER [ execute_1 ] END
			...
			recv_fetch_repasseData ( ref block_detalhe_loja, ref block_sumario_loja ); 
