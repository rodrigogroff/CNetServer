...
{
	#if DEBUG 
	public class tst_exec_excluiCart : tst_base
	{
/// USER [ usr_var_decl ]
/// USER [ usr_var_decl ] END

		public exec_excluiCart transaction = new exec_excluiCart();
		...
		public tst_exec_excluiCart ( ref StreamWriter p_m_Log, 
		...
		public bool Test()
		{
			if ( demand == "!" || demand == "exec_excluiCart" )
			{
			...
			{
				result = " exec_excluiCart                    " + " ## no available coverage! ";
				currentItem++;
				return false;
			}
		...
		public void call_exec_excluiCart ( string emp, string mat, string tit, ref CNetHeader header ) 
		{
			m_Comm.Clear();
			
			DataPortable send_dp = new DataPortable();
			
			send_dp.MapTagValue ( COMM_IN_EXEC_EXCLUICART.emp, emp );
			send_dp.MapTagValue ( COMM_IN_EXEC_EXCLUICART.mat, mat );
			send_dp.MapTagValue ( COMM_IN_EXEC_EXCLUICART.tit, tit );
			
			m_Comm.AddEntryPortable ( ref send_dp );
			
			DataPortable send_dp_cont_0 = new DataPortable();
			
			send_dp_cont_0.MapTagContainer ( COMM_IN_EXEC_EXCLUICART.header, header as DataPortable );
			
			m_Comm.AddEntryPortable ( ref send_dp_cont_0 );
		}

		public void recv_exec_excluiCart (  )
		{
		}
...
/// USER [ custom_functions ] END
		...
		public bool Item1()
		{
			...
			tst_unit.LogTest ( "exec_excluiCart Item1", ref m_Log );
...
/// USER [ setup_test_1 ] END
			...
			call_exec_excluiCart ( emp, mat, tit, ref  header ); 
