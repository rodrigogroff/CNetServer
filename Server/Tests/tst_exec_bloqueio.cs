...
{
	#if DEBUG 
	public class tst_exec_bloqueio : tst_base
	{
/// USER [ usr_var_decl ]
/// USER [ usr_var_decl ] END

		public exec_bloqueio transaction = new exec_bloqueio();
		...
		public tst_exec_bloqueio ( ref StreamWriter p_m_Log, 
		...
		public bool Test()
		{
			if ( demand == "!" || demand == "exec_bloqueio" )
			{
			...
			{
				result = " exec_bloqueio                      " + " ## no available coverage! ";
				currentItem++;
				return false;
			}
		...
		public void call_exec_bloqueio ( string st_cartao, string st_motivo, ref CNetHeader header ) 
		{
			m_Comm.Clear();
		...
		public void recv_exec_bloqueio (  )
		{
		}
...
/// USER [ custom_functions ] END
		...
		public bool Item1()
		{
			...
			tst_unit.LogTest ( "exec_bloqueio Item1", ref m_Log );
...
/// USER [ setup_test_1 ] END
			...
			call_exec_bloqueio ( st_cartao, st_motivo, ref  header ); 
