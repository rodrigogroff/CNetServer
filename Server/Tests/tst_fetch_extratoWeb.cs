...
{
	#if DEBUG 
	public class tst_fetch_extratoWeb : tst_base
	{
/// USER [ usr_var_decl ]
/// USER [ usr_var_decl ] END

		public fetch_extratoWeb transaction = new fetch_extratoWeb();
		...
		public tst_fetch_extratoWeb ( ref StreamWriter p_m_Log, 
		...
		public bool Test()
		{
			if ( demand == "!" || demand == "fetch_extratoWeb" )
			{
			...
			{
				result = " fetch_extratoWeb                   " + " ## no available coverage! ";
				currentItem++;
				return false;
			}
		...
		public void call_fetch_extratoWeb ( string st_cartao, string st_senha ) 
		{
			m_Comm.Clear();
...
/// USER [ custom_functions ] END
		...
		public bool Item1()
		{
			...
			tst_unit.LogTest ( "fetch_extratoWeb Item1", ref m_Log );
...
/// USER [ setup_test_1 ] END
			...
			call_fetch_extratoWeb ( st_cartao, st_senha ); 
...
/// USER [ execute_1 ] END
			...
			recv_fetch_extratoWeb ( ref st_content, ref vr_disp, ref vr_lim ); 
