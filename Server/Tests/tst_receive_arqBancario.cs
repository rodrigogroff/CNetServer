...
{
	#if DEBUG || LOAD
	public class tst_receive_arqBancario : tst_base
	{
/// USER [ usr_var_decl ]
/// USER [ usr_var_decl ] END

		public receive_arqBancario transaction = new receive_arqBancario();
		...
		public tst_receive_arqBancario ( ref StreamWriter p_m_Log, 
		...
		public bool Test()
		{
			...
			if ( demand == "!" || demand == "receive_arqBancario" )
			{
				#if DEBUG
			...
			{
				result = " receive_arqBancario                " + " ## no available coverage! ";
				currentItem++;
				return false;
			}
		...
		public void recv_receive_arqBancario ( ref string st_new_identif )
		{
			DataPortable recv_dp = m_Comm.GetFirstExitPortable(); 
			
			recv_dp.GetMapValue ( COMM_OUT_RECEIVE_ARQBANCARIO.st_new_identif , ref st_new_identif );
		}
...
/// USER [ custom_functions ] END
		...
		public bool Item1()
		{
			...
			tst_unit.LogTest ( "receive_arqBancario Item1", ref m_Log );
...
/// USER [ setup_test_1 ] END
			...
			call_receive_arqBancario ( st_identif, ref  header, ref lst ); 
...
/// USER [ execute_1 ] END
			...
			string st_new_identif = "";

			recv_receive_arqBancario ( ref st_new_identif ); 
