...
{
	#if DEBUG 
	public class tst_fetch_comprov_Gift : tst_base
	{
/// USER [ usr_var_decl ]
/// USER [ usr_var_decl ] END

		public fetch_comprov_Gift transaction = new fetch_comprov_Gift();
		...
		public tst_fetch_comprov_Gift ( ref StreamWriter p_m_Log, 
		...
		public bool Test()
		{
			if ( demand == "!" || demand == "fetch_comprov_Gift" )
			{
			...
			{
				result = " fetch_comprov_Gift                 " + " ## no available coverage! ";
				currentItem++;
				return false;
			}
		...
		public void call_fetch_comprov_Gift ( string id_gift, string tg_reimp, ref CNetHeader header ) 
		{
			m_Comm.Clear();
...
/// USER [ custom_functions ] END
		...
		public bool Item1()
		{
			...
			tst_unit.LogTest ( "fetch_comprov_Gift Item1", ref m_Log );
...
/// USER [ setup_test_1 ] END
			...
			call_fetch_comprov_Gift ( id_gift, tg_reimp, ref  header ); 
...
/// USER [ execute_1 ] END
			...
			recv_fetch_comprov_Gift ( ref lst ); 
