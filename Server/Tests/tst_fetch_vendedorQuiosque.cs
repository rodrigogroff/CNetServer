...
{
	#if DEBUG 
	public class tst_fetch_vendedorQuiosque : tst_base
	{
/// USER [ usr_var_decl ]
/// USER [ usr_var_decl ] END

		public fetch_vendedorQuiosque transaction = new fetch_vendedorQuiosque();
		...
		public tst_fetch_vendedorQuiosque ( ref StreamWriter p_m_Log, 
		...
		public bool Test()
		{
			if ( demand == "!" || demand == "fetch_vendedorQuiosque" )
			{
			...
			{
				result = " fetch_vendedorQuiosque             " + " ## no available coverage! ";
				currentItem++;
				return false;
			}
		...
		public void call_fetch_vendedorQuiosque ( string st_empresa, string st_desc, ref CNetHeader header ) 
		{
			m_Comm.Clear();
...
/// USER [ custom_functions ] END
		...
		public bool Item1()
		{
			...
			tst_unit.LogTest ( "fetch_vendedorQuiosque Item1", ref m_Log );
...
/// USER [ setup_test_1 ] END
			...
			call_fetch_vendedorQuiosque ( st_empresa, st_desc, ref  header ); 
...
/// USER [ execute_1 ] END
			...
			recv_fetch_vendedorQuiosque ( ref lstVinc, ref lstOutros ); 
