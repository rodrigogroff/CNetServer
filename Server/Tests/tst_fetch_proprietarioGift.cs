...
{
	#if DEBUG 
	public class tst_fetch_proprietarioGift : tst_base
	{
/// USER [ usr_var_decl ]
/// USER [ usr_var_decl ] END

		public fetch_proprietarioGift transaction = new fetch_proprietarioGift();
		...
		public tst_fetch_proprietarioGift ( ref StreamWriter p_m_Log, 
		...
		public bool Test()
		{
			if ( demand == "!" || demand == "fetch_proprietarioGift" )
			{
			...
			{
				result = " fetch_proprietarioGift             " + " ## no available coverage! ";
				currentItem++;
				return false;
			}
		...
		public void call_fetch_proprietarioGift ( string st_matricula, ref CNetHeader header ) 
		{
			m_Comm.Clear();
		...
		public void recv_fetch_proprietarioGift ( ref DadosProprietario dp )
		{
			DataPortable recv_dp_cont_dp = new DataPortable();
			
			m_Comm.GetExitPortableAtPosition (0).GetMapContainer ( COMM_OUT_FETCH_PROPRIETARIOGIFT.dp , ref recv_dp_cont_dp );
...
/// USER [ custom_functions ] END
		...
		public bool Item1()
		{
			...
			tst_unit.LogTest ( "fetch_proprietarioGift Item1", ref m_Log );
...
/// USER [ setup_test_1 ] END
			...
			call_fetch_proprietarioGift ( st_matricula, ref  header ); 
...
/// USER [ execute_1 ] END
			...
			recv_fetch_proprietarioGift ( ref dp ); 
