...
{
	#if DEBUG 
	public class tst_fetch_reciboRepasse : tst_base
	{
/// USER [ usr_var_decl ]
/// USER [ usr_var_decl ] END

		public fetch_reciboRepasse transaction = new fetch_reciboRepasse();
		...
		public tst_fetch_reciboRepasse ( ref StreamWriter p_m_Log, 
		...
		public bool Test()
		{
			if ( demand == "!" || demand == "fetch_reciboRepasse" )
			{
			...
			{
				result = " fetch_reciboRepasse                " + " ## no available coverage! ";
				currentItem++;
				return false;
			}
		...
		public void call_fetch_reciboRepasse ( string id_repasse, ref CNetHeader header ) 
		{
			m_Comm.Clear();
...
/// USER [ custom_functions ] END
		...
		public bool Item1()
		{
			...
			tst_unit.LogTest ( "fetch_reciboRepasse Item1", ref m_Log );
...
/// USER [ setup_test_1 ] END
			...
			call_fetch_reciboRepasse ( id_repasse, ref  header ); 
...
/// USER [ execute_1 ] END
			...
			DadosRepRecibo rec = new DadosRepRecibo();
			ArrayList lst = new ArrayList();

			recv_fetch_reciboRepasse ( ref rec, ref lst ); 
