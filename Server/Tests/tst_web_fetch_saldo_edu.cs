...
{
	#if DEBUG 
	public class tst_web_fetch_saldo_edu : tst_base
	{
/// USER [ usr_var_decl ]
/// USER [ usr_var_decl ] END

		public web_fetch_saldo_edu transaction = new web_fetch_saldo_edu();
		...
		public tst_web_fetch_saldo_edu ( ref StreamWriter p_m_Log, 
		...
		public bool Test()
		{
			if ( demand == "!" || demand == "web_fetch_saldo_edu" )
			{
			...
			{
				result = " web_fetch_saldo_edu                " + " ## no available coverage! ";
				currentItem++;
				return false;
			}
		...
		public void call_web_fetch_saldo_edu ( string st_cartao, string st_senha, string tg_resp ) 
		{
			m_Comm.Clear();
