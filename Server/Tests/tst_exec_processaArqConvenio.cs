...
{
	#if DEBUG 
	public class tst_exec_processaArqConvenio : tst_base
	{
/// USER [ usr_var_decl ]
/// USER [ usr_var_decl ] END

		public exec_processaArqConvenio transaction = new exec_processaArqConvenio();
		...
		public tst_exec_processaArqConvenio ( ref StreamWriter p_m_Log, 
		...
		public bool Test()
		{
			if ( demand == "!" || demand == "exec_processaArqConvenio" )
			{
			...
			{
				result = " exec_processaArqConvenio           " + " ## no available coverage! ";
				currentItem++;
				return false;
			}
			...
			result = " exec_processaArqConvenio            " + 
		...
		public void call_exec_processaArqConvenio ( string st_id, string st_empresa, ref CNetHeader header ) 
		{
			m_Comm.Clear();
		...
		public void recv_exec_processaArqConvenio (  )
		{
		}
...
/// USER [ custom_functions ] END
		...
		public bool Item1()
		{
			...
			tst_unit.LogTest ( "exec_processaArqConvenio Item1", ref m_Log );
...
/// USER [ setup_test_1 ] END
			...
			call_exec_processaArqConvenio ( st_id, st_empresa, ref  header ); 
