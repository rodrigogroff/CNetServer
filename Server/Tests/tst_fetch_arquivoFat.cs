...
{
	#if DEBUG 
	public class tst_fetch_arquivoFat : tst_base
	{
/// USER [ usr_var_decl ]
/// USER [ usr_var_decl ] END

		public fetch_arquivoFat transaction = new fetch_arquivoFat();
		...
		public tst_fetch_arquivoFat ( ref StreamWriter p_m_Log, 
		...
		public bool Test()
		{
			if ( demand == "!" || demand == "fetch_arquivoFat" )
			{
			...
			{
				result = " fetch_arquivoFat                   " + " ## no available coverage! ";
				currentItem++;
				return false;
			}
		...
		public void call_fetch_arquivoFat ( string dia_venc_ini, string dia_venc_fim, string tg_debito, ref CNetHeader header ) 
		{
			m_Comm.Clear();
...
/// USER [ custom_functions ] END
		...
		public bool Item1()
		{
			...
			tst_unit.LogTest ( "fetch_arquivoFat Item1", ref m_Log );
...
/// USER [ setup_test_1 ] END
			...
			call_fetch_arquivoFat ( dia_venc_ini, dia_venc_fim, tg_debito, ref  header ); 
...
/// USER [ execute_1 ] END
			...
			recv_fetch_arquivoFat ( ref st_msg, ref nu_nsFat ); 
