...
{
	#if DEBUG 
	public class tst_fetch_usuarios : tst_base
	{
/// USER [ usr_var_decl ]
/// USER [ usr_var_decl ] END

		public fetch_usuarios transaction = new fetch_usuarios();
		...
		public tst_fetch_usuarios ( ref StreamWriter p_m_Log, 
		...
		public bool Test()
		{
			if ( demand == "!" || demand == "fetch_usuarios" )
			{
			...
			{
				result = " fetch_usuarios                     " + " ## no available coverage! ";
				currentItem++;
				return false;
			}
		...
		public void call_fetch_usuarios ( string st_csv_list, ref CNetHeader header ) 
		{
			m_Comm.Clear();
		...
		public void recv_fetch_usuarios ( ref string st_csv )
		{
			DataPortable recv_dp = m_Comm.GetFirstExitPortable(); 
			
			recv_dp.GetMapValue ( COMM_OUT_FETCH_USUARIOS.st_csv , ref st_csv );
		}
...
/// USER [ custom_functions ] END
		...
		public bool Item1()
		{
			...
			tst_unit.LogTest ( "fetch_usuarios Item1", ref m_Log );
...
/// USER [ setup_test_1 ] END
			...
			call_fetch_usuarios ( st_csv_list, ref  header ); 
...
/// USER [ execute_1 ] END
			...
			recv_fetch_usuarios ( ref st_csv ); 
