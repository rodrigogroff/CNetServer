...
{
	#if DEBUG 
	public class tst_fetch_terminais : tst_base
	{
/// USER [ usr_var_decl ]
/// USER [ usr_var_decl ] END

		public fetch_terminais transaction = new fetch_terminais();
		...
		public tst_fetch_terminais ( ref StreamWriter p_m_Log, 
		...
		public bool Test()
		{
			if ( demand == "!" || demand == "fetch_terminais" )
			{
			...
			{
				result = " fetch_terminais                    " + " ## no available coverage! ";
				currentItem++;
				return false;
			}
		...
		public void call_fetch_terminais ( string st_csvId, ref CNetHeader header ) 
		{
			m_Comm.Clear();
...
/// USER [ custom_functions ] END
		...
		public bool Item1()
		{
			...
			tst_unit.LogTest ( "fetch_terminais Item1", ref m_Log );
...
/// USER [ setup_test_1 ] END
			...
			call_fetch_terminais ( st_csvId, ref  header ); 
