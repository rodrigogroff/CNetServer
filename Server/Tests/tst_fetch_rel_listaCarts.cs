...
{
	#if DEBUG 
	public class tst_fetch_rel_listaCarts : tst_base
	{
/// USER [ usr_var_decl ]
/// USER [ usr_var_decl ] END

		public fetch_rel_listaCarts transaction = new fetch_rel_listaCarts();
		...
		public tst_fetch_rel_listaCarts ( ref StreamWriter p_m_Log, 
		...
		public bool Test()
		{
			if ( demand == "!" || demand == "fetch_rel_listaCarts" )
			{
			...
			{
				result = " fetch_rel_listaCarts               " + " ## no available coverage! ";
				currentItem++;
				return false;
			}
		...
		public void call_fetch_rel_listaCarts ( string emp, ref CNetHeader header ) 
		{
			m_Comm.Clear();
			
			DataPortable send_dp = new DataPortable();
			
			send_dp.MapTagValue ( COMM_IN_FETCH_REL_LISTACARTS.emp, emp );
			
			m_Comm.AddEntryPortable ( ref send_dp );
			
			DataPortable send_dp_cont_0 = new DataPortable();
			
			send_dp_cont_0.MapTagContainer ( COMM_IN_FETCH_REL_LISTACARTS.header, header as DataPortable );
			
			m_Comm.AddEntryPortable ( ref send_dp_cont_0 );
		}

		public void recv_fetch_rel_listaCarts ( ref string id, ref string nome_emp )
		{
			DataPortable recv_dp = m_Comm.GetFirstExitPortable(); 
			
			recv_dp.GetMapValue ( COMM_OUT_FETCH_REL_LISTACARTS.id , ref id );
			recv_dp.GetMapValue ( COMM_OUT_FETCH_REL_LISTACARTS.nome_emp , ref nome_emp );
		}
...
/// USER [ custom_functions ] END
		...
		public bool Item1()
		{
			...
			tst_unit.LogTest ( "fetch_rel_listaCarts Item1", ref m_Log );
...
/// USER [ setup_test_1 ] END
			...
			call_fetch_rel_listaCarts ( emp, ref  header ); 
...
/// USER [ execute_1 ] END
			...
			recv_fetch_rel_listaCarts ( ref id, ref nome_emp ); 
