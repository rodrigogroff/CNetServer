...
{
	#if DEBUG 
	public class tst_exec_desbloq_empresa : tst_base
	{
/// USER [ usr_var_decl ]
/// USER [ usr_var_decl ] END

		public exec_desbloq_empresa transaction = new exec_desbloq_empresa();
		...
		public tst_exec_desbloq_empresa ( ref StreamWriter p_m_Log, 
		...
		public bool Test()
		{
			if ( demand == "!" || demand == "exec_desbloq_empresa" )
			{
			...
			{
				result = " exec_desbloq_empresa               " + " ## no available coverage! ";
				currentItem++;
				return false;
			}
		...
		public void call_exec_desbloq_empresa ( string emp, ref CNetHeader header ) 
		{
			m_Comm.Clear();
			
			DataPortable send_dp = new DataPortable();
			
			send_dp.MapTagValue ( COMM_IN_EXEC_DESBLOQ_EMPRESA.emp, emp );
			
			m_Comm.AddEntryPortable ( ref send_dp );
			
			DataPortable send_dp_cont_0 = new DataPortable();
			
			send_dp_cont_0.MapTagContainer ( COMM_IN_EXEC_DESBLOQ_EMPRESA.header, header as DataPortable );
			
			m_Comm.AddEntryPortable ( ref send_dp_cont_0 );
		}

		public void recv_exec_desbloq_empresa (  )
		{
		}
...
/// USER [ custom_functions ] END
		...
		public bool Item1()
		{
			...
			tst_unit.LogTest ( "exec_desbloq_empresa Item1", ref m_Log );
...
/// USER [ setup_test_1 ] END
			...
			call_exec_desbloq_empresa ( emp, ref  header ); 
