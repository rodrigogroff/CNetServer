...
{
	#if DEBUG 
	public class tst_fetch_relFat : tst_base
	{
/// USER [ usr_var_decl ]
/// USER [ usr_var_decl ] END

		public fetch_relFat transaction = new fetch_relFat();
		...
		public tst_fetch_relFat ( ref StreamWriter p_m_Log, 
		...
		public bool Test()
		{
			...
			{
				result = " fetch_relFat                       " + " ## no available coverage! ";
				currentItem++;
				return false;
			}
		...
		public void call_fetch_relFat ( string st_dt_ini, string st_dt_fim, string tg_type, ref CNetHeader header ) 
		{
			m_Comm.Clear();
...
/// USER [ custom_functions ] END
		...
		public bool Item1()
		{
			...
			tst_unit.LogTest ( "fetch_relFat Item1", ref m_Log );
...
/// USER [ setup_test_1 ] END
			...
			call_fetch_relFat ( st_dt_ini, st_dt_fim, tg_type, ref  header ); 
...
/// USER [ execute_1 ] END
			...
			recv_fetch_relFat ( ref st_total, ref st_content_block ); 
