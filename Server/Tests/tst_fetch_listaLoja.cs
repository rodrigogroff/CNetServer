...
{
	#if DEBUG || LOAD
	public class tst_fetch_listaLoja : tst_base
	{
/// USER [ usr_var_decl ]
/// USER [ usr_var_decl ] END

		public fetch_listaLoja transaction = new fetch_listaLoja();
		...
		public tst_fetch_listaLoja ( ref StreamWriter p_m_Log, 
		...
		public bool Test()
		{
			...
			if ( demand == "!" || demand == "fetch_listaLoja" )
			{
				#if DEBUG
			...
			{
				result = " fetch_listaLoja                    " + " ## no available coverage! ";
				currentItem++;
				return false;
			}
		...
		public void call_fetch_listaLoja ( string st_csv, ref CNetHeader header ) 
		{
			m_Comm.Clear();
			
			DataPortable send_dp = new DataPortable();
			
			send_dp.MapTagValue ( COMM_IN_FETCH_LISTALOJA.st_csv, st_csv );
			
			m_Comm.AddEntryPortable ( ref send_dp );
			
			DataPortable send_dp_cont_0 = new DataPortable();
			
			send_dp_cont_0.MapTagContainer ( COMM_IN_FETCH_LISTALOJA.header, header as DataPortable );
			
			m_Comm.AddEntryPortable ( ref send_dp_cont_0 );
		}

		public void recv_fetch_listaLoja ( ref ArrayList list )
		{
			DataPortable recv_dp_array_generic_list = m_Comm.GetExitPortableAtPosition (0);
			
			recv_dp_array_generic_list.GetMapArray ( COMM_OUT_FETCH_LISTALOJA.list , ref list );
		}
...
/// USER [ custom_functions ] END
		...
		public bool Item1()
		{
			...
			tst_unit.LogTest ( "fetch_listaLoja Item1", ref m_Log );
...
/// USER [ setup_test_1 ] END
			...
			call_fetch_listaLoja ( st_csv, ref  header ); 
