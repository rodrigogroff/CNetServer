...
{
	#if DEBUG 
	public class tst_exec_cotaExtraEmpresa : tst_base
	{
/// USER [ usr_var_decl ]
/// USER [ usr_var_decl ] END

		public exec_cotaExtraEmpresa transaction = new exec_cotaExtraEmpresa();
		...
		public tst_exec_cotaExtraEmpresa ( ref StreamWriter p_m_Log, 
		...
		public bool Test()
		{
			if ( demand == "!" || demand == "exec_cotaExtraEmpresa" )
			{
			...
			{
				result = " exec_cotaExtraEmpresa              " + " ## no available coverage! ";
				currentItem++;
				return false;
			}
		...
		public void call_exec_cotaExtraEmpresa ( string emp, string valor, string csv_excluded, ref CNetHeader header ) 
		{
			m_Comm.Clear();
			
			DataPortable send_dp = new DataPortable();
			
			send_dp.MapTagValue ( COMM_IN_EXEC_COTAEXTRAEMPRESA.emp, emp );
			send_dp.MapTagValue ( COMM_IN_EXEC_COTAEXTRAEMPRESA.valor, valor );
			send_dp.MapTagValue ( COMM_IN_EXEC_COTAEXTRAEMPRESA.csv_excluded, csv_excluded );
			
			m_Comm.AddEntryPortable ( ref send_dp );
			
			DataPortable send_dp_cont_0 = new DataPortable();
			
			send_dp_cont_0.MapTagContainer ( COMM_IN_EXEC_COTAEXTRAEMPRESA.header, header as DataPortable );
			
			m_Comm.AddEntryPortable ( ref send_dp_cont_0 );
		}

		public void recv_exec_cotaExtraEmpresa (  )
		{
		}
...
/// USER [ custom_functions ] END
		...
		public bool Item1()
		{
			...
			tst_unit.LogTest ( "exec_cotaExtraEmpresa Item1", ref m_Log );
...
/// USER [ setup_test_1 ] END
			...
			call_exec_cotaExtraEmpresa ( emp, valor, csv_excluded, ref  header ); 
