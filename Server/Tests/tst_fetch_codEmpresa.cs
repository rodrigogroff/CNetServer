...
{
	#if DEBUG || LOAD
	public class tst_fetch_codEmpresa : tst_base
	{
/// USER [ usr_var_decl ]
/// USER [ usr_var_decl ] END

		public fetch_codEmpresa transaction = new fetch_codEmpresa();
		...
		public tst_fetch_codEmpresa ( ref StreamWriter p_m_Log, 
		...
		public bool Test()
		{
			...
			if ( demand == "!" || demand == "fetch_codEmpresa" )
			{
				#if DEBUG
			...
			{
				result = " fetch_codEmpresa                   " + " ## no available coverage! ";
				currentItem++;
				return false;
			}
		...
		public void call_fetch_codEmpresa ( ref CNetHeader header ) 
		{
			m_Comm.Clear();
			
			DataPortable send_dp_cont_0 = new DataPortable();
			
			send_dp_cont_0.MapTagContainer ( COMM_IN_FETCH_CODEMPRESA.header, header as DataPortable );
			
			m_Comm.AddEntryPortable ( ref send_dp_cont_0 );
		}

		public void recv_fetch_codEmpresa ( ref string st_empresa )
		{
			DataPortable recv_dp = m_Comm.GetFirstExitPortable(); 
			
			recv_dp.GetMapValue ( COMM_OUT_FETCH_CODEMPRESA.st_empresa , ref st_empresa );
		}
...
/// USER [ custom_functions ] END
		...
		public bool Item1()
		{
			...
			tst_unit.LogTest ( "fetch_codEmpresa Item1", ref m_Log );
...
/// USER [ setup_test_1 ] END
			...
			call_fetch_codEmpresa ( ref  header ); 
...
/// USER [ execute_1 ] END
			...
			recv_fetch_codEmpresa ( ref st_empresa ); 
