...
{
	#if DEBUG 
	public class tst_del_extraGift : tst_base
	{
/// USER [ usr_var_decl ]
/// USER [ usr_var_decl ] END

		public del_extraGift transaction = new del_extraGift();
		...
		public tst_del_extraGift ( ref StreamWriter p_m_Log, 
		...
		public bool Test()
		{
			if ( demand == "!" || demand == "del_extraGift" )
			{
			...
			{
				result = " del_extraGift                      " + " ## no available coverage! ";
				currentItem++;
				return false;
			}
		...
		public void recv_del_extraGift (  )
		{
		}
...
/// USER [ custom_functions ] END
		...
		public bool Item1()
		{
			...
			tst_unit.LogTest ( "del_extraGift Item1", ref m_Log );
...
/// USER [ setup_test_1 ] END
			...
			call_del_extraGift ( id, ref  header ); 
