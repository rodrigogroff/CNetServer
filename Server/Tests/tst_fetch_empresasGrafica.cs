...
{
	#if DEBUG 
	public class tst_fetch_empresasGrafica : tst_base
	{
/// USER [ usr_var_decl ]
/// USER [ usr_var_decl ] END

		public fetch_empresasGrafica transaction = new fetch_empresasGrafica();
		...
		public tst_fetch_empresasGrafica ( ref StreamWriter p_m_Log, 
		...
		public bool Test()
		{
			if ( demand == "!" || demand == "fetch_empresasGrafica" )
			{
			...
			{
				result = " fetch_empresasGrafica              " + " ## no available coverage! ";
				currentItem++;
				return false;
			}
...
/// USER [ custom_functions ] END
		...
		public bool Item1()
		{
			...
			tst_unit.LogTest ( "fetch_empresasGrafica Item1", ref m_Log );
...
/// USER [ setup_test_1 ] END
			...
			call_fetch_empresasGrafica ( ref  header ); 
...
/// USER [ execute_1 ] END
			...
			recv_fetch_empresasGrafica ( ref lst ); 
