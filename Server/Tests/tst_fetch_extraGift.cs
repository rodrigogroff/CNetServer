...
{
	#if DEBUG 
	public class tst_fetch_extraGift : tst_base
	{
/// USER [ usr_var_decl ]
/// USER [ usr_var_decl ] END

		public fetch_extraGift transaction = new fetch_extraGift();
		...
		public tst_fetch_extraGift ( ref StreamWriter p_m_Log, 
		...
		public bool Test()
		{
			if ( demand == "!" || demand == "fetch_extraGift" )
			{
			...
			{
				result = " fetch_extraGift                    " + " ## no available coverage! ";
				currentItem++;
				return false;
			}
		...
		public void recv_fetch_extraGift ( ref ArrayList lst )
		{
...
/// USER [ custom_functions ] END
		...
		public bool Item1()
		{
			...
			tst_unit.LogTest ( "fetch_extraGift Item1", ref m_Log );
...
/// USER [ setup_test_1 ] END
			...
			call_fetch_extraGift ( ref  header ); 
