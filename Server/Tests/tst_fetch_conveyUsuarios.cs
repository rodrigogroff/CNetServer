...
{
	#if DEBUG 
	public class tst_fetch_conveyUsuarios : tst_base
	{
/// USER [ usr_var_decl ]
/// USER [ usr_var_decl ] END

		public fetch_conveyUsuarios transaction = new fetch_conveyUsuarios();
		...
		public tst_fetch_conveyUsuarios ( ref StreamWriter p_m_Log, 
		...
		public bool Test()
		{
			if ( demand == "!" || demand == "fetch_conveyUsuarios" )
			{
			...
			{
				result = " fetch_conveyUsuarios               " + " ## no available coverage! ";
				currentItem++;
				return false;
			}
...
/// USER [ custom_functions ] END
		...
		public bool Item1()
		{
			...
			tst_unit.LogTest ( "fetch_conveyUsuarios Item1", ref m_Log );
...
/// USER [ setup_test_1 ] END
			...
			call_fetch_conveyUsuarios ( ref  header ); 
...
/// USER [ execute_1 ] END
			...
			recv_fetch_conveyUsuarios ( ref lst ); 
