...
{
	#if DEBUG || LOAD
	public class tst_fetch_listaEmpresas : tst_base
	{
/// USER [ usr_var_decl ]
/// USER [ usr_var_decl ] END

		public fetch_listaEmpresas transaction = new fetch_listaEmpresas();
		...
		public tst_fetch_listaEmpresas ( ref StreamWriter p_m_Log, 
		...
		public bool Test()
		{
			...
			if ( demand == "!" || demand == "fetch_listaEmpresas" )
			{
				#if DEBUG
			...
			{
				result = " fetch_listaEmpresas                " + " ## no available coverage! ";
				currentItem++;
				return false;
			}
		...
		public void call_fetch_listaEmpresas ( string st_csv_empresas, ref CNetHeader header ) 
		{
			m_Comm.Clear();
			
			DataPortable send_dp = new DataPortable();
			
			send_dp.MapTagValue ( COMM_IN_FETCH_LISTAEMPRESAS.st_csv_empresas, st_csv_empresas );
			
			m_Comm.AddEntryPortable ( ref send_dp );
			
			DataPortable send_dp_cont_0 = new DataPortable();
			
			send_dp_cont_0.MapTagContainer ( COMM_IN_FETCH_LISTAEMPRESAS.header, header as DataPortable );
			
			m_Comm.AddEntryPortable ( ref send_dp_cont_0 );
		}

		public void recv_fetch_listaEmpresas ( ref ArrayList lst )
		{
			DataPortable recv_dp_array_generic_lst = m_Comm.GetExitPortableAtPosition (0);
			
			recv_dp_array_generic_lst.GetMapArray ( COMM_OUT_FETCH_LISTAEMPRESAS.lst , ref lst );
		}
...
/// USER [ custom_functions ] END
		...
		public bool Item1()
		{
			...
			tst_unit.LogTest ( "fetch_listaEmpresas Item1", ref m_Log );
...
/// USER [ setup_test_1 ] END
			...
			call_fetch_listaEmpresas ( st_csv_empresas, ref  header ); 
...
/// USER [ execute_1 ] END
			...
			recv_fetch_listaEmpresas ( ref lst ); 
