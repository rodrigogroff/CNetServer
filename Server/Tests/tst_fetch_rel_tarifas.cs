...
{
	#if DEBUG 
	public class tst_fetch_rel_tarifas : tst_base
	{
/// USER [ usr_var_decl ]
/// USER [ usr_var_decl ] END

		public fetch_rel_tarifas transaction = new fetch_rel_tarifas();
		...
		public tst_fetch_rel_tarifas ( ref StreamWriter p_m_Log, 
		...
		public bool Test()
		{
			if ( demand == "!" || demand == "fetch_rel_tarifas" )
			{
			...
			{
				result = " fetch_rel_tarifas                  " + " ## no available coverage! ";
				currentItem++;
				return false;
			}
...
/// USER [ custom_functions ] END
		...
		public bool Item1()
		{
			...
			tst_unit.LogTest ( "fetch_rel_tarifas Item1", ref m_Log );
...
/// USER [ setup_test_1 ] END
			...
			call_fetch_rel_tarifas ( ref  header ); 
...
/// USER [ execute_1 ] END
			...
			recv_fetch_rel_tarifas ( ref st_csv, ref st_csv_emp ); 
