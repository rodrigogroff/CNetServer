...
{
	#if DEBUG 
	public class tst_fetch_dadosAluno : tst_base
	{
/// USER [ usr_var_decl ]
/// USER [ usr_var_decl ] END

		public fetch_dadosAluno transaction = new fetch_dadosAluno();
		...
		public tst_fetch_dadosAluno ( ref StreamWriter p_m_Log, 
		...
		public bool Test()
		{
			if ( demand == "!" || demand == "fetch_dadosAluno" )
			{
			...
			{
				result = " fetch_dadosAluno                   " + " ## no available coverage! ";
				currentItem++;
				return false;
			}
		...
		public void call_fetch_dadosAluno ( string st_cartao, ref CNetHeader header ) 
		{
			m_Comm.Clear();
...
/// USER [ custom_functions ] END
		...
		public bool Item1()
		{
			...
			tst_unit.LogTest ( "fetch_dadosAluno Item1", ref m_Log );
...
/// USER [ setup_test_1 ] END
			...
			call_fetch_dadosAluno ( st_cartao, ref  header ); 
