...
{
	#if DEBUG || LOAD
	public class tst_fetch_dadosProprietario : tst_base
	{
/// USER [ usr_var_decl ]
/// USER [ usr_var_decl ] END

		public fetch_dadosProprietario transaction = new fetch_dadosProprietario();
		...
		public tst_fetch_dadosProprietario ( ref StreamWriter p_m_Log, 
		...
		public bool Test()
		{
			...
			if ( demand == "!" || demand == "fetch_dadosProprietario" )
			{
				#if DEBUG
			...
			{
				result = " fetch_dadosProprietario            " + " ## no available coverage! ";
				currentItem++;
				return false;
			}
			...
			result = " fetch_dadosProprietario             " + 
		...
		public void call_fetch_dadosProprietario ( string cpf_cnpj, ref CNetHeader header ) 
		{
			m_Comm.Clear();
			
			DataPortable send_dp = new DataPortable();
			
			send_dp.MapTagValue ( COMM_IN_FETCH_DADOSPROPRIETARIO.cpf_cnpj, cpf_cnpj );
			
			m_Comm.AddEntryPortable ( ref send_dp );
			
			DataPortable send_dp_cont_0 = new DataPortable();
			
			send_dp_cont_0.MapTagContainer ( COMM_IN_FETCH_DADOSPROPRIETARIO.header, header as DataPortable );
			
			m_Comm.AddEntryPortable ( ref send_dp_cont_0 );
		}

		public void recv_fetch_dadosProprietario ( ref DadosProprietario dp )
		{
			DataPortable recv_dp_cont_dp = new DataPortable();
			
			m_Comm.GetExitPortableAtPosition (0).GetMapContainer ( COMM_OUT_FETCH_DADOSPROPRIETARIO.dp , ref recv_dp_cont_dp );
...
/// USER [ custom_functions ] END
		...
		public bool Item1()
		{
			...
			tst_unit.LogTest ( "fetch_dadosProprietario Item1", ref m_Log );
...
/// USER [ setup_test_1 ] END
			...
			call_fetch_dadosProprietario ( cpf_cnpj, ref  header ); 
...
/// USER [ execute_1 ] END
			...
			recv_fetch_dadosProprietario ( ref dp ); 
