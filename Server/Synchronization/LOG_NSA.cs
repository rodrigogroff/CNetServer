...
{
	public class LOG_NSA : Synchronization
	{
		...
		public LOG_NSA ()
		{
			var_Serial = TB_LOG_NSA.serial;
			var_Tablename = TB_LOG_NSA.Alias;
		}
		
		public LOG_NSA ( Transaction trx )
		{
		...
		public void copy ( ref LOG_NSA cpy )
		{
			dt_log = cpy.dt_log;
		}
		...
		{
			i_unique = row.GetField ( TB_LOG_NSA.i_unique );
			dt_log = ConvertTime ( row.GetField ( TB_LOG_NSA.dt_log ) );
		}
		
		public bool synchronize_LOG_NSA ( )
		{
			try{
			...
			if ( m_hshChangedFields [ "dt_log" ] != null )
				m_gen_dbStatement.AddUpdate ( TB_LOG_NSA.dt_log , dt_log , TB_LOG_NSA.type_dt_log );
		
			m_gen_dbStatement.AddWhere ( TB_LOG_NSA.i_unique, i_unique, TB_LOG_NSA.type_i_unique );
		...
		{
			m_gen_dbStatement.AddSelect ( TB_LOG_NSA.i_unique );
			m_gen_dbStatement.AddSelect ( TB_LOG_NSA.dt_log );
		}
		...
		public bool selectIdentity ( string identity )
		{
			do
			{
				...
				m_gen_dbStatement.AddWhere ( TB_LOG_NSA.i_unique, identity, TB_LOG_NSA.type_i_unique );
		...
		public bool create_LOG_NSA ( )
		{
			...
			m_gen_dbStatement.AddValue ( TB_LOG_NSA.dt_log, dt_log, TB_LOG_NSA.type_dt_log );
