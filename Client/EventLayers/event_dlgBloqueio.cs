...
namespace Client
{
	public class event_dlgBloqueio : EventLayer
	{
		#region - EVENTS -
		...
		public event_dlgBloqueio ( ApplicationUtil util, Exchange ex )
		{
			var_util = util;
			var_exchange = ex;
		
			i_Form = new dlgBloqueio ( this );
		}
		...
		public override bool doEvent ( int event_number, object arg )
		{
			switch ( event_number )
			{
				...
				case event_FormIsOpening:
				{
					...
					i_Form.CboBloq.Items.AddRange ( new MotivoBloqueioDesc().GetArray().ToArray() );
					i_Form.CboBloq.SelectedIndex = 0;
				...
				case event_Confirmar:
				{
					...
					if ( i_Form.CboBloq.SelectedIndex.ToString() == MotivoBloqueio.CANCELAMENTO )
					{
						if ( MessageBox.Show (	"Este cartão não poderá ser desbloqueado. Confirma?",
					                    		"Aviso", 
					...
					var_exchange.exec_bloqueio ( 	ctrl_TxtEmpresa.getTextBoxValue().PadLeft ( 6, '0' ) +
					                            	ctrl_TxtMatricula.getTextBoxValue().PadLeft ( 6, '0' ) + 
					                            	ctrl_TxtTit.getTextBoxValue().PadLeft ( 2, '0' ), 
					                            	i_Form.CboBloq.SelectedIndex.ToString(), 
					                            	ref header );
